 ...
 * <P>
 * This version of Thinlet is the enhanced one by <A HREF="http://www.zee.ch/thinlet/">Zee</A>
 * With further modifications by Mike Hartshorn (m.hartshorn@astex-therapeutics.com)
 * 
 * Add action to tab. Put action in dtd and invoke it whenever the tabbedpane action gets called.
 */
package thinlet;
 ...
 *
 */
public class Thinlet extends Canvas
  ...
  private static final int MINIMUM_COLUMN_WIDTH = 10;
  private static final int DEFAULT_RESIZE_BORDER_COLOR = 0x4040ff;
  private static final String PROPERTY_SMARTWIDTHS = ":smartwidths";
    ...
    private static final int DEFAULT_RIGHT  = 2;
	...
	private transient Color c_resizeborder;
	...
	}
	{
		...
		setColors(0xe6e6e6, 0x000000, 0xffffff,
			0x909090, 0xb0b0b0, 0xededed, 0xb9b9b9, 0x89899a, 0xc5c5dd, DEFAULT_RESIZE_BORDER_COLOR);
	 ...
	 * @param resizeBorder used as color for the border decorator for resizable dialogs.
	...
	public void setColors(int background, int text, int textbackground,
			...
			int focus, int select, int resizeBorder) {
		...
		c_resizeborder = new Color(resizeBorder);
	...
	public void setColors(int background, int text, int textbackground,
		...
		setColors(background,text,textbackground,border,disable,hover,press,focus,select,DEFAULT_RESIZE_BORDER_COLOR);
	}
	...
	private void doLayout(Object component) {
		...
		if ("combobox".equals(classname)) {
			...
			int gap = getInteger(component, "gap", DEFAULT_GAP);
			...
			if (grid != null) { // has subcomponents
				...
				top = getInteger(component, "top", DEFAULT_TOP);
				left = getInteger(component, "left", DEFAULT_LEFT);
				int bottom = getInteger(component, "bottom", DEFAULT_BOTTOM);
				int right = getInteger(component, "right", DEFAULT_RIGHT);
			...
			if ("table".equals(classname)) {
				...
				if (header != null) {
					...
					for (int i = 0; i < columnwidths.length; i++) {
						...
						columnwidths[i] = getInteger(column, "width", DEFAULT_COLUMN_WIDTH);
        ...
        if(header!=null && getBoolean(header,"resizable")) {
          ...
          for(int i=0;i<smartWidths.length;i++) smartWidths[i]=MINIMUM_COLUMN_WIDTH;
	...
	private void showTip() {
		...
		else if (":combolist".equals(classname) && insidepart instanceof Object[]) {
			text = getString(insidepart, "tooltip", null);
		}
	...
	private Dimension getPreferredSize(Object component) {
		...
		if ("tabbedpane".equals(classname)) {
			...
			for (Object tab = get(component, ":comp");
				...
				if ("stacked".equals(placement)) { tabsize += d.height + 3; }
		...
		if (("panel".equals(classname)) || ("dialog".equals(classname))) {
			...
			size.width += getInteger(component, "left", DEFAULT_LEFT) + getInteger(component, "right", DEFAULT_RIGHT);
			size.height += getInteger(component, "top", DEFAULT_TOP) + getInteger(component, "bottom", DEFAULT_BOTTOM);
			...
			for (Object comp = get(component, ":comp");
					comp != null; comp = get(comp, ":next")) {
				String iclass = getClass(comp);
				if ((!"dialog".equals(iclass)) && (!":popup".equals(iclass)) &&
						(!":combolist".equals(iclass))) {
					Dimension d = getPreferredSize(comp);
	...
	private Dimension getSize(Object component, int dx, int dy) {
            // force size specfication
            int w = getInteger(component, "width", -1);
            int h = getInteger(component, "height", -1);
            if(w != -1 && h != -1){
    ...
    private void setupPaintImage(int w, int h){

        if(paintImage==null ||
           (paintImage.getWidth(this) != w &&
            paintImage.getHeight(this) != h)){
            
            if(paintImage != null) paintImage.flush();
            paintImage=createImage(w,h);
            if(paintGraphics != null) paintGraphics.dispose();
            paintGraphics=paintImage.getGraphics();
        }
    }
    ...
    private void resetBuffer(){
        ...
        offscreen=bufferImage.getGraphics();
    }
	...
	public void paint(Graphics dg) {
        ...
        // or initialises the image with the first paint
            if(bufferWidth != getSize().width ||
               bufferHeight != getSize().height || 
           bufferImage == null || offscreen == null){
            resetBuffer();
        }

            //Graphics g = doublebuffer.getGraphics();
		offscreen.setClip(dg.getClipBounds());
		...
		//g.fillRect(0, 0, getNetWeight().width, getNetWeight().height);
		//long time = System.currentTimeMillis();
		Rectangle clip = offscreen.getClipBounds();
		///dg.setClip(r.x, r.y, r.width, r.height);
		paint(offscreen, clip.x, clip.y, clip.width, clip.height, content, isEnabled());
		//System.out.println(System.currentTimeMillis() - time);
		//g.setClip(0, 0, getNetWeight().width, getNetWeight().height);
		//g.setColor(Color.red); g.drawRect(clip.x, clip.y, clip.width - 1, clip.height - 1);
		//g.dispose();
		dg.drawImage(bufferImage, 0, 0, this);

	}

    /* clear out the double buffer as we changed size. */
	...
	private void paint(Graphics g,
		...
		g.translate(bounds.x, bounds.y); 
                ...
                // try to stop graphics getting screwed if something
                // bad happens in the repaint cycle
                try {
		...
		if (	"label".equals(classname)) {
			...
			paintField(g, clipx, clipy, clipwidth, clipheight, component,
				bounds.width, bounds.height, focus, enabled, ("passwordfield".equals(classname)), 0);
		}
		else if ("textarea".equals(classname)) {
			paintScroll(component, classname, pressed, inside, focus, enabled,
				g, clipx, clipy, clipwidth, clipheight);
			...
			for (Object tab = get(component, ":comp"); tab != null; tab = get(tab, ":next")) {
				...
				if (selected != i) {
					...
					paint(tab, r.x + bx, r.y + by, r.width - bw, r.height - bh,
						...
						!stacked && (!"top".equals(placement)), (!"left".equals(placement)),
						1, 3, 1, 3, false, tabenabled ? (hover ? 'h' : 'g') : 'd', "left", true, false);
										
				} else {
					selectedtab = tab;
					// paint tabbedpane border
					paint(tab, ("left".equals(placement)) ? r.width - 1 : 0,
						stacked ? (r.y + r.height - 1) : ("top".equals(placement)) ? r.height - 1 : 0,
			...
			if (	"dialog".equals(classname)) {
				...
				if(getBoolean(component, "resizable", false)) {
					...
					g.fillRect(bounds.width-3,bounds.height-16,3,16);
					g.fillRect(bounds.width-16,bounds.height-3,16,3);
				}
			...
			for (Object menu = get(get(component, "menu"), ":comp");
				...
				if ("separator".equals(itemclass)) {
					...
					paint(menu, r.x, r.y, bounds.width - 2, r.height,
						g, clipx, clipy, clipwidth, clipheight, false, false, false, false,
						2, ("checkboxmenuitem".equals(itemclass)) ? (block + 7) : 4, 2, 4, false,
	...
	private void paintScroll(Object component, String classname,
		...
		if ((!"panel".equals(classname)) && (!"dialog".equals(classname)) &&
				((!"textarea".equals(classname)) || getBoolean(component, "border", true))) {
			...
			if ("table".equals(classname)) {
                ...
                enabled ? (insidepart==column?'h':'g') : 'd', "left", false, false);
  ...
  private void setSmartWidth(Object column) {
	...
	private void paint(Object component,
		...
		if ("textarea".equals(classname)) {
			...
			Object lead = get(component, ":lead");
			int[] columnwidths = ("table".equals(classname)) ? ((int []) get(component, ":widths")) : null;
			boolean line = getBoolean(component, "line", true); int iline = line ? 1 : 0;
			boolean angle = ("tree".equals(classname)) && getBoolean(component, "angle", false);
			for (Object item = get(component, ":comp"), next = null; item != null; item = next) {
				...
				paintRect(g, (!"tree".equals(classname)) ? 0 : r.x, r.y,
					(!"tree".equals(classname)) ? viewwidth : r.width, r.height, c_focus,
            ...
            //storing widhts for smartwidth feature
            //unfortunatly, for instance, we have to do the width stuff twice, will be done in paint as well
            Object header = get(component,"header");
            if(header!=null && getBoolean(header,"resizable")) {
              String text = getString(cell,"text");
              Image icon = getIcon(component, "icon", null);
              int smartWidth = 2; //TODO define a constant, should be equal to the spacing between cells
              if(text != null) smartWidth += g.getFontMetrics().stringWidth(text);
              if(icon != null) smartWidth += icon.getWidth(this) + (text!=null?2:0);
              int [] smartWidths = (int [])get(component,PROPERTY_SMARTWIDTHS);
              if(smartWidth>smartWidths[i]) smartWidths[i]=smartWidth;
            }
						//column width is defined by header calculated in layout, otherwise is DEFAULT_COLUMN_WIDTH
						int iwidth = DEFAULT_COLUMN_WIDTH;
	...
	private void paint(Object component, int x, int y, int width, int height,
		...
		if (focus) {
			...
			g.drawRect(x + 1, y + 1, width - 3, height - 3);
                ...
                Method paintMethod = (Method)get(component, "paint");

		String text = getString(component, "text", null);
		Image icon = getIcon(component, "icon", null);
		if ((text == null) && (icon == null) && (paintMethod == null)) { return; }
		...
		int cx = x;
		if ("center".equals(alignment)) { cx += (width - tw - iw) / 2; }
			else if ("right".equals(alignment)) { cx += width - tw - iw; }
		...
		if (icon != null) {
			...
			cx += iw;
                ...
                // this craziness works around some problem with
                // setClip/getClip in jdk1.1.8
                if(paintMethod != null){
                    setupPaintImage(width-2, height-2);
			...
			if (id == MouseEvent.MOUSE_ENTERED) {
				...
				if (popupowner != null) { // remove popup
					...
					if ((popupowner != mouseinside) &&
							(!":popup".equals(classname)) && (!":combolist".equals(classname))) {
						closeup();
					}
				}
	 ...
	 */
		...
		if ("button".equals(classname)) {
			...
			return processField(component, shiftdown, controldown, modifiers,
				keychar, keycode, false, ("passwordfield".equals(classname)), false);
		}
		else if ("textarea".equals(classname)) {
			char[] chars = (char[]) get(component, ":text");
			int start = getInteger(component, "start", 0);
			...
			if ((keycode == KeyEvent.VK_RIGHT) || (keycode == KeyEvent.VK_DOWN) ||
				...
				if (newvalue != selected) {
					...
					invoke(component, getItem(component, newvalue), "action");
                                        ...
                                        // mjh
					invoke(getItem(component, newvalue), null, "action");
				}
			}
		}
		else if ("spinbox".equals(classname)) {
			...
			//? for interval mode select its all subnode or deselect all after
				...
				if (node != null) {
					if (getBoolean(lead, "expanded", true)) { // select its first subnode
                                            selectItem(component, node, true, false);
						setLead(component, lead, node);
					}
					else { // expand
						setBoolean(lead, "expanded", true, true);
						selectItem(component, lead, true, false);
						validate(component);
			...
			return processList(component, shiftdown, controldown, keychar, keycode, true);
                    ...
                    Component awtComponent = getComponent(component, "bean");

                    if(awtComponent instanceof KeyListener){
                        ((KeyListener)awtComponent).keyPressed(new KeyEvent(this,KeyEvent.KEY_PRESSED,System.currentTimeMillis(),modifiers,keycode, (char)keychar));
                    }
			...
			if ((keycode == KeyEvent.VK_UP) || (keycode == KeyEvent.VK_DOWN)) {
				...
				else if ("menubar".equals(classname)) { // select the previous menubar menu
				...
				else if ((selected != null) && ("menu".equals(getClass(selected)))) { // expand menu
					Object popup = popupMenu(last);
					set(popup, "selected", getMenu(popup, null, true, true));
				}
				else if ("menubar".equals(classname)) { // select the next menubar menu
				...
				if ((keycode != KeyEvent.VK_ESCAPE) &&
					...
					if ((selected != null) && ("checkboxmenuitem".equals(getClass(selected)))) {
	...
	private Object getMenu(Object component, Object part,
		...
		for (int i = 0; i < 2; i++) { // 0: next to last, 1: first to previous
			for (Object item = (i == 0) ? get(part, ":next") :
				...
				if ((!"separator".equals(getClass(item))) && getBoolean(item, "enabled", true)) {
	...
	private boolean processField(Object component,
		...
		if (filter && (insert != null)) { // contributed by Michael Nascimento
			for (int i = insert.length() - 1; i >= 0; i--) {
                            // mjh, allow insert of . into a spinbox
                            if (!Character.isDigit(insert.charAt(i)) && insert.charAt(i) != '.') { return false; }
			}
		}
	...
	private boolean processList(Object component, boolean shiftdown, boolean controldown,
		...
		if ((keycode == KeyEvent.VK_UP) || // select previous/next/first/... item
			...
			if (((keycode == KeyEvent.VK_A) || (keycode == 0xBF)) && //KeyEvent.VK_SLASH
					(!"single".equals(getString(component, "selection", "single")))) { // select all
    ...
    // mjh, much fiddling about to stop tree expansion 
    private void selectItem(Object component, Object row, boolean recursive) {
        selectItem(component, row, recursive, true);
    }
	 ...
	 * @param recursive true for tree
	 */
    private void selectItem(Object component, Object row, boolean recursive, boolean select) {
		boolean changed = false;
		for (Object item = get(component, ":comp");
				item != null; item = getNextItem(component, item, recursive)) {
		...
		set(component, ":anchor", null);
                // mjh, stop tree expand firing the action
		//if (changed && getClass(component) != "tree") { invoke(component, row, "action"); }
		if (changed && select) { invoke(component, row, "action"); }
	}
	...
	private void handleMouseEvent(int x, int y, int clickcount,
        ...
        Object resizeComponent = null;
				...
				if(header!=null && (resizeComponent=get(header,":resizecomponent"))!=null) {
					noScroll=true;
          if(clickcount == 2) {
            ...
            repaint(component);
					  ...
					  if(id == MouseEvent.MOUSE_PRESSED) {
  						...
  						//resize the column, but limit its minimum size to a width of MINIMUM_COLUMN_WIDTH
              int offset = x - referencex;

  						int newSize = getInteger(resizeComponent, "width") + (offset>=0?offset:offset);
				...
				if(header!=null && get(header,"action") != null) {
					if(insidepart!=null && insidepart instanceof Object[] && "column".equals(getClass(insidepart))) {
						noScroll=true;
						if ( id == MouseEvent.MOUSE_ENTERED ||
					...
					if (((id == MouseEvent.MOUSE_PRESSED)||
						...
						for (Object item = get(component, ":comp"); item != null;) {
							...
							if (my < r.y + r.height) {
								...
								if (mx < r.x) {
									if ((mx >= r.x - block) && (get(item, ":comp") != null)) {
										...
										selectItem(component, item, true, false);
							...
							if ((id != MouseEvent.MOUSE_DRAGGED) ||
								...
								select(component, item, ("tree".equals(classname)), shiftdown, controldown);
								if (clickcount == 2) { invoke(component, item, "perform"); }
							}
							break;
						}
						item = getNextItem(component, item, ("tree".equals(classname)));
					}
				}
			}
		}
			...
			if(id==MouseEvent.MOUSE_PRESSED ||
				...
				id==MouseEvent.MOUSE_DRAGGED) {
                            //int cx = getInteger(component, "__clipx__", 0);
                            //int cy = getInteger(component, "__clipy__", 0);
                            //System.out.println("clipx " + cx);
                            //System.out.println("clipy " + cy);
		...
		else if (("button".equals(classname)) || ("bean".equals(classname)) || ("label".equals(classname)) ||
			...
			processField(x, y, clickcount, id, component,
				false, ("passwordfield".equals(classname)), 0);
		}
		else if ("textarea".equals(classname)) {
			if (!processScroll(x, y, id, component, part)) {
			...
			if (((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_EXITED)) &&
				...
				if ((part != insidepart) && ((insidepart == null) ||
						((insidepart instanceof Object[]) && (!"menu".equals(getClass(insidepart)))))) {
					if ((insidepart != null) && getBoolean(insidepart, "enabled", true)) {
						if ("checkboxmenuitem".equals(getClass(insidepart))) {
			...
			if (part != null) {
				if (((id == MouseEvent.MOUSE_ENTERED) || (id == DRAG_ENTERED)) &&
					...
					if ((insidepart == null) || (!"menu".equals(getClass(insidepart)))) {
						if ((insidepart != null) && getBoolean(insidepart, "enabled", true)) {
							if ("checkboxmenuitem".equals(getClass(insidepart))) {
								changeCheck(insidepart, false);
							}
    ...
    private boolean processSpin(Object component, Object part) {
        String text = getString(component, "text", "");
        int decimals = getInteger(component, "decimals", 0);
    ...
    /** Decide if we should accept a spinbox change on basis of inc/decrement. */
    private String acceptChange(String text, String step, String limit, int dp, int dir){
        int itext = unformat(text, dp);
        int istep = unformat(step, dp);
        int ilimit = unformat(limit, dp);

        if((dir ==  1 && itext + istep <= ilimit) ||
           (dir == -1 && itext - istep >= ilimit)){
            itext += dir * istep;

            return format(itext, dp);
        }
        return null;
    }

    /**
     * Convert from spinbox string representation to integer.
     *
     * @param s  The current spinbox text value.
     * @param dp The number of decimal places assumed.
     */
    public static int unformat(String s, int dp) throws NumberFormatException {
        if(dp == 0){
        ...
        // insufficient dp so multiply in value
        while(actualDps++ < dp){
            val *= 10;
        }
    ...
    /**
     * Turn integer value back into suitable String for the spinbox.
     * Manual formatting is used rather than DecimalFormat as this
     * seems to be very slow the first time one is created.
     *
     * No internationalization of the decimal place character is carried out.
     *
     * @param v  The new spinbox int value.
     * @param dp The number of decimal places we want.
     */
    private static String format(int v, int dp){
        if(dp == 0){
        ...
        // add in any leading zeros
        for(int i = len; i < dp + 1; i++){
            s = "0" + s;
        }

        len = s.length();

        s = s.substring(0, len - dp) + "." + s.substring(len - dp);
	...
	private boolean setNextFocusable(Object current, boolean outgo) {
		...
		for (Object next = null, component = current; true; component = next) {
			...
			while (next == null) { // find the next of the parents, or the topmost
				...
				if ((component == content) || (("dialog".equals(getClass(component))) &&
	...
	private boolean isFocusable(Object component, boolean forced) {
		String classname = getClass(component);
		if (("bean".equals(classname)) ||
                    ("button".equals(classname)) || ("checkbox".equals(classname)) || ("togglebutton".equals(classname)) ||
				...
				("list".equals(classname)) || ("table".equals(classname)) || ("tree".equals(classname)) ||
				("tabbedpane".equals(classname)) || (forced && ("splitpane".equals(classname)))) {
			for (Object comp = component; comp != null;) {
				...
				if (("tab".equals(getClass(comp))) && (getItem(parent,
	...
	public int getSelectedIndex(Object component) {
		...
		if (("combobox".equals(classname)) || ("tabbedpane".equals(classname))) {
			return getInteger(component, "selected", ("combobox".equals(classname)) ? -1 : 0);
		}
		if (("list".equals(classname)) || ("table".equals(classname)) || ("header".equals(classname)) || ("tree".equals(classname))) {
    ...
    private void insertItem(Object parent, Object key, Object component, int index) {
        ...
        if(useArrayLists && key.equals(":comp")){
            ...
            if(v == null){
                ...
                int size = 4;
                if(childCountHint != null){
                    size = Integer.parseInt(childCountHint);
    ...
    public Object shallowFind(String name){
        return shallowFind(content, name);
    }
	
	/**
	 * Finds the first component from the specified component by a name
         * Does not descend into a table's row and cell children
	 *
	 * @param component the widget is searched inside this component
	 * @param name parameter value identifies the widget
	 * @return the first suitable component, or null
	 */
	public Object shallowFind(Object component, String name) {
		if (name.equals(get(component, "name"))) {
			return component;
		}

                // we are a table and we weren't the object, go back
	...
	private boolean checkMnemonic(Object component,
		...
		else if (("menuitem".equals(classname)) || ("checkboxmenuitem".equals(classname))) {
		...
		// check parent
		if (parent && ((!"dialog".equals(classname)) || !getBoolean(component, "modal", false))) {
			if (checkMnemonic(getParent(component), true,
					("tab".equals(classname)) ? checked : component, keycode, modifiers)) { return true; }
		}
		return false;
	}
...
/**
     ...
     * They must have the following signature currently.
     *
     *   void paintMethod(Thinlet t, Object component,
     *                    Graphics g, int w, int h);
     */
    private void setPaintMethod(Object component, String methodName){
        try {
            ...
            set(component, "paint", paintMethod);
        }catch(Exception e){
            throw new IllegalArgumentException(methodName + "(Thinlet,Object,Graphics,int,int) not found");
        }
    }
			...
			for (int i = 0; i < methods.size(); i += 3) {
				...
				if ("method" == definition[0]) {
                                    if("paint" == definition[1]){
                                        setPaintMethod(component, value);
                                    }else{
	...
	private void addImpl(Object parent, Object component, int index) {
		...
		if ((("combobox".equals(parentclass)) && ("choice".equals(classname))) ||
				...
				((("menu".equals(parentclass)) || ("popupmenu".equals(parentclass))) &&
					(("menu".equals(classname)) || ("menuitem".equals(classname)) ||
					("checkboxmenuitem".equals(classname)) || ("separator".equals(classname)))) ||
				((("panel".equals(parentclass)) || ("desktop".equals(parentclass)) ||
					("splitpane".equals(parentclass)) || ("dialog".equals(parentclass)) ||
					("tab".equals(parentclass))) && instance(classname, "component") &&
						(!"popupmenu".equals(classname)))) {
			insertItem(parent, ":comp", component, index);
			set(component, ":parent", parent);
		}
		else if ((("table".equals(parentclass)) && ("header".equals(classname))) ||
				(("popupmenu".equals(classname)) && instance(parentclass, "component"))) {
	 ...
	 * use the other <code>setFont</code> method instead
	 */
	public Font getFont(Object component) { // deprecated
            Font thisFont = (Font)get(component, "font");

            return (thisFont == null) ? font : thisFont;
	}
		...
		if (value != null) {
			...
			try {
				...
				while (st.hasMoreTokens()) {
					...
					try {
							modifiers |= InputEvent.class.getField(token + "_MASK").getInt(null);
		...
		for (int i = 0; i < n; i++) {
			...
			if ("thinlet".equals(arg)) {
				...
				if ("item".equals(compname)) {
					...
					if ("list".equals(parentclass)) { classname = "item"; }
					else if ("tree".equals(parentclass)) { classname = "node"; }
					else if ("table".equals(parentclass)) { classname = "row"; }
					else if ("combobox".equals(parentclass)) { classname = "choice"; }
					else if ("tabbedpane".equals(parentclass)) { classname = "tab"; }
	...
	private void update(Object component, Object mode) {
		...
		while (component != null) {
			...
			else if (("paint" == mode) && ("tabbedpane".equals(classname))) {
			...
			if (("layout" == mode) || (("validate" == mode) &&
					...
					("tree".equals(classname)) || ("dialog".equals(classname)) || (parent == content)))) {
